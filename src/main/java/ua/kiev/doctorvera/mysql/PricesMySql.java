...
public class PricesMySql extends AbstractMySql<Prices, Integer> {
	...
	@Override
	protected void prepareStatementForInsert(PreparedStatement statement, Prices price)
	...
	@Override
	protected void prepareStatementForUpdate(PreparedStatement statement, Prices price)
			throws PersistException {
		try {
			statement.setFloat(1, price.getTotal());
			statement.setDate(2, new java.sql.Date(price.getDateTime().getTime()));
	...
	public Collection<Prices> findByTotalBetween(Float min, Float max) {
		...
		for (Prices price : pricesList) {
			if (min <= price.getTotal() && price.getTotal() <= max)
				result.add(price);
		}
		return result;
	}
