...
public class PlanMySql extends AbstractMySql<Plan, Integer> {
	...
	@Override
	protected void prepareStatementForInsert(PreparedStatement statement, Plan plan)
	...
	@Override
	protected void prepareStatementForUpdate(PreparedStatement statement, Plan plan)
			throws PersistException {
		try {
			statement.setDate(1, new java.sql.Date(plan.getDateTimeStart().getTime()));
			statement.setDate(2, new java.sql.Date(plan.getDateTimeEnd().getTime()));
	...
	public Collection<Plan> findByDoctor(Users doctor) {
	...
	public Collection<Plan> findByRoom(Rooms room) {
		ArrayList<Plan> planList = new ArrayList<Plan>();
		try {
	...
	public Collection<Plan> findByTime(Date date) {
		List<Plan> planList;
		try {
			planList = findAll();
		} catch (PersistException e) {
		...
		for (Plan plan : planList) {
			if (plan.getDateTimeStart().before(date) && plan.getDateTimeEnd().before(date))
				result.add(plan);
		}
		return result;
	}

	public Collection<Plan> findByTimeBetween(Date min, Date max) {
		List<Plan> planList;
		try {
			planList = findAll();
		} catch (PersistException e) {
		...
		ArrayList<Plan> result = new ArrayList<Plan>();
		for (Plan plan : planList) {
			if ((plan.getDateTimeStart().after(min) && plan.getDateTimeStart().before(max))
	...
	public Collection<Plan> findByDescription(String description) {
		ArrayList<Plan> planList = new ArrayList<Plan>();
		try {
