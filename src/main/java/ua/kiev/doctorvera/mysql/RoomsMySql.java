...
public class RoomsMySql extends AbstractMySql<Rooms, Integer> {
	...
	@Override
	protected void prepareStatementForInsert(PreparedStatement statement, Rooms room)
	...
	@Override
	protected void prepareStatementForUpdate(PreparedStatement statement, Rooms room)
			throws PersistException {
		try {
			statement.setString(1, room.getName());
			statement.setString(2, room.getDescription());
	...
	public Collection<Rooms> findByName(String name) {
	...
	public Collection<Rooms> findByDescription(String description) {
		ArrayList<Rooms> roomsList = new ArrayList<Rooms>();
		try {
